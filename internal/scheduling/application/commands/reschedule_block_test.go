...
func TestRescheduleBlockHandler_Handle(t *testing.T) {
	userID := uuid.New()
	date := time.Date(2024, time.January, 15, 0, 0, 0, 0, time.UTC)

	t.Run("successfully reschedules a block", func(t *testing.T) {
		repo := new(mockScheduleRepo)
		outboxRepo := new(mockSchedulingOutboxRepo)
		uow := new(mockSchedulingUnitOfWork)
		handler := NewRescheduleBlockHandler(repo, outboxRepo, uow)
	...
	t.Run("fails with invalid time range", func(t *testing.T) {
		...
		// End before start
		newStart := time.Date(2024, time.January, 15, 15, 0, 0, 0, time.UTC)
		newEnd := time.Date(2024, time.January, 15, 14, 0, 0, 0, time.UTC)
...
func TestNewRescheduleBlockHandler(t *testing.T) {
	repo := new(mockScheduleRepo)
	outboxRepo := new(mockSchedulingOutboxRepo)
	uow := new(mockSchedulingUnitOfWork)
