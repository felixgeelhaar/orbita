...
func TestOrbit_RegisterTools(t *testing.T) {
	...
	assert.NotEmpty(t, tools, "expected focus mode tools to be registered")

	// Check for expected tool names (as registered in tools.go)
	expectedTools := []string{"start", "end", "cancel", "status", "list", "stats", "settings_get", "settings_update", "suggest"}
...
func TestOrbit_RegisterCommands(t *testing.T) {
	...
	// Focus mode doesn't register CLI commands
	commands := harness.GetRegisteredCommands()
	assert.Empty(t, commands, "focus mode orbit should not register CLI commands")
}
...
func TestOrbit_SubscribeEvents(t *testing.T) {
	...
	// Test event handling - emit a task completed event
	err = harness.EmitEvent("core.task.completed", map[string]any{
		"task_id": "test-task-id",
	})
	require.NoError(t, err)

	// Test schedule block started event
	err = harness.EmitEvent("scheduling.block.started", map[string]any{
